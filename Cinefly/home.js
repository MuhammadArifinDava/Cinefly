        ...
        fetch(`https://api.themoviedb.org/3/${endpoint}&api_key=${api_Key}`)
            ...
            .then(data => {
                ...
                fetchResults.forEach(item => {
                    ...
                    const media_Type = item.media_type || (containerClass === 'netflix-container' ? 'tv' : 'movie');
                    
                    itemElement.addEventListener('click', () => {
                        fetch('getSessionRole.php')
                          .then(res => res.text())
                          .then(role => {
                            if (role.trim() === 'guest') {
                              window.location.href = 'register.php';  
                            } else {
                              window.location.href = `movie_details/movie_details.php?media=${media_Type}&id=${item.id}`;  // Allow others to proceed
                            }
                          });
                    });
                });
                ...
                if (containerClass === 'netflix-container') {
                    ...
                    function redirectToMovieDetails() {
                        const media_Type = randomMovie.media_type || 'tv'; 
                        window.location.href = `movie_details/movie_details.php?media=${media_Type}&id=${randomMovie.id}`;
                    }
...
function displaySearchResults(results) {
    ...
    results.map(movie => {
        ...
        (thumbnail && info).addEventListener('click', () => {
            ...
            window.location.href = `movie_details/movie_details.php?media=${media_Type}&id=${movie.id}`;
        });
